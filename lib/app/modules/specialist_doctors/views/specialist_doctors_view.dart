...
class _SpecialistDoctorsGrid extends StatelessWidget {
  ...
  bool _isHospitalBookingCategory() {
    ...
    final label = (args['categoryLabel'] ?? '').toString().toLowerCase().trim();
    final key = (args['categoryKey'] ?? '').toString().toLowerCase().trim();

    return label.contains('hospital booking') ||
        label.contains('hospitals booking') ||
        label.contains('হাসপাতাল বুকিং') ||
        key.contains('hospital_booking') ||
        key.contains('hospitals_booking');
  }
