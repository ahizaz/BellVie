...
import 'package:bellevie/app/modules/home/widgets/sections/banner_details_page.dart';
import 'package:cached_network_image/cached_network_image.dart';
import 'package:flutter/foundation.dart';
import 'package:flutter/material.dart';
...
class SliderOneBannerItem {
  final int id;
  final String image;

  const SliderOneBannerItem({
    required this.id,
    required this.image,
  });
}
...
class _HomeBannerCarouselState extends State<HomeBannerCarousel> {
  ...
  List<SliderOneBannerItem> _apiBanners = <SliderOneBannerItem>[];
  ...
  void initState() {
    ...
    if (_useApiBanners) {
      ...
      _apiBanners = _localBanners
          .asMap()
          .entries
          .map(
            (e) => SliderOneBannerItem(
              id: e.key,
              image: e.value,
            ),
          )
  ...
  Future<void> _loadCachedBanners() async {
    ...
    final banners = _parseSliderOneBanners(results);
  ...
  List<SliderOneBannerItem> _parseSliderOneBanners(List<dynamic> results) {
    return results
        .whereType<Map<String, dynamic>>()
        .map((item) {
          ...
          return SliderOneBannerItem(
            id: id,
            image: image,
          );
        })
        .whereType<SliderOneBannerItem>()
        .toList();
  }
    ...
    try {
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        ...
        if (banners.isNotEmpty) {
          final isDifferent = _apiBanners.length != banners.length ||
              _apiBanners.asMap().entries.any(
                    (e) =>
                        banners[e.key].id != e.value.id ||
                        banners[e.key].image != e.value.image,
                  );
              ...
              child: _isInitialLoading
                  ...
                  : hasBanners
                      ? PageView.builder(
                          ...
                          onPageChanged: (i) => setState(() => _index = i),
                          itemBuilder: (_, i) {
                            final banner = _apiBanners[i];
                            final source = banner.image;
                            final isNetwork = source.startsWith('http://') ||
                                source.startsWith('https://');
